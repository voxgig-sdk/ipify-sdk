...
describe('GetPublicIpDirect', async () => {
  ...
  test('direct-load-get_public_ip', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-get_public_ip', setup.live)) return
