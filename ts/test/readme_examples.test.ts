...
//     (\`new IpifySDK(...)\` / \`IpifySDK.test(...)\` become
//     \`IpifySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"get_public_ip":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new IpifySDK(...)\` and any
// \`IpifySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+IpifySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/IpifySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
