...
(0, node_test_1.describe)('GetPublicIpEntity', async () => {
    // Per-test live pacing. Delay is read from sdk-test-control.json's
    // `test.live.delayMs`; only sleeps when IPIFY_TEST_LIVE=TRUE.
    (0, node_test_1.afterEach)((0, utility_1.liveDelay)('IPIFY_TEST_LIVE'));
    (0, node_test_1.test)('instance', async () => {
        const testsdk = __1.IpifySDK.test();
    ...
    (0, node_test_1.test)('basic', async (t) => {
        ...
        let get_public_ip_ref01_data = Object.values(setup.data.existing.get_public_ip)[0];
        // LOAD
        const get_public_ip_ref01_ent = client.GetPublicIp();
        const get_public_ip_ref01_match_dt0 = {};
        const get_public_ip_ref01_data_dt0 = (await get_public_ip_ref01_ent.load(get_public_ip_ref01_match_dt0)).data();
        (0, node_assert_1.default)(null != get_public_ip_ref01_data_dt0);
    });
});
function basicSetup(extra) {
    ...
    const entityDataFile = node_path_1.default.resolve(__dirname, '../../../../.sdk/test/entity/get_public_ip/GetPublicIpTestData.json');
    // TODO: file ready util needed?
    const entityDataSource = Fs.readFileSync(entityDataFile).toString('utf8');
    // TODO: need a xlang JSON parse utility in voxgig/struct with better error msgs
    const entityData = JSON.parse(entityDataSource);
    options.entity = entityData.existing;
    let client = __1.IpifySDK.test(options, extra);
    const struct = client.utility().struct;
    const merge = struct.merge;
    const transform = struct.transform;
    let idmap = transform(['get_public_ip01', 'get_public_ip02', 'get_public_ip03'], {
        '`$PACK`': ['', {
                '`$KEY`': '`$COPY`',
...
//# sourceMappingURL=GetPublicIpEntity.test.js.map