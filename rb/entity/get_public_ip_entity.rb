...
class GetPublicIpEntity
  ...
  def make
    opts = @_entopts.dup
    GetPublicIpEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = IpifyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [GetPublicIp, Hash] the current GetPublicIp data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = IpifyHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of GetPublicIp fields)
  def match_get
  ...
  #
  # @param reqmatch [GetPublicIpLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.GetPublicIp.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [GetPublicIp, Hash] the loaded GetPublicIp; raises IpifyError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = IpifyHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
