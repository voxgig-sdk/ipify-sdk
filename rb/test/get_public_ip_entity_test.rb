...
class GetPublicIpEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = get_public_ip_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["load"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "get_public_ip." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set IPIFY_TEST_GET_PUBLIC_IP_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    get_public_ip_ref01_data = nil
    if get_public_ip_ref01_data_raw.length > 0
...
def get_public_ip_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "get_public_ip", "GetPublicIpTestData.json")
  ...
  entid_env_raw = ENV["IPIFY_TEST_GET_PUBLIC_IP_ENTID"]
  ...
  if env["IPIFY_TEST_LIVE"] == "TRUE"
    ...
    client = IpifySDK.new(Helpers.to_map(merged_opts))
  end
