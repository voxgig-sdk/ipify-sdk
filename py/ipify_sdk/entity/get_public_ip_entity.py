...
from ipify_sdk.ipify_types import (
    GetPublicIp,
...
class GetPublicIpEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return GetPublicIpEntity(self._client, opts)
    ...
    def match_get(self) -> GetPublicIp:
    ...
    def load(self, reqmatch=None, ctrl=None) -> GetPublicIp:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.GetPublicIp().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
