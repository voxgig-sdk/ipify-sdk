...
class IpifyTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            IpifyTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = IpifyTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = IpifyTestRunner.getenv("IPIFY_TEST_LIVE")
        override = IpifyTestRunner.getenv("IPIFY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = IpifyTestRunner.getenv(key)
        ...
        explain = IpifyTestRunner.getenv("IPIFY_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["IPIFY_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if IpifyTestRunner._test_control is not None:
            return IpifyTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                IpifyTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            IpifyTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = IpifyTestRunner.load_test_control()
...
def load_env_local():
    IpifyTestRunner.load_env_local()


def env_override(m):
    return IpifyTestRunner.env_override(m)


def entity_data(v):
    return IpifyTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return IpifyTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return IpifyTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return IpifyTestRunner.load_test_control()


def live_client_options():
    return IpifyTestRunner.live_client_options()


def live_delay_ms():
    return IpifyTestRunner.live_delay_ms()
