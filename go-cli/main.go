// Command Ipify-cli is a CLI + REPL for the Ipify SDK,
...
const entitiesHelp = "get_public_ip"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      IPIFY_APIKEY (api key), IPIFY_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: IPIFY_APIKEY carries the API key and
	// IPIFY_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("IPIFY_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("IPIFY_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewIpifySDK(opts)
