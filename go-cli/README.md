...
boru-driven command-line client **and** interactive REPL for the Ipify
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ipify-cli)
make build
...
./ipify-cli load 1 get_public_ip            # {id:1} shorthand
./ipify-cli load '{id:1}' get_public_ip       # explicit match map

# 5. Override the API base URL for a single call
IPIFY_BASE=https://api.example.com ./ipify-cli load 1 get_public_ip

# 6. No arguments -> interactive REPL
./ipify-cli
ipify> load 1 get_public_ip
ipify> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ipify-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/ipify-cli load 1 get_public_ip
   ```
...
```sh
./ipify-cli load 1 get_public_ip          # scalar shorthand for {id:1}
./ipify-cli load '{id:1}' get_public_ip     # explicit match map
```
...
./ipify-cli load 1 get_public_ip
```
...
Run with no arguments to open a REPL (prompt `ipify>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `get_public_ip`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/ipify-cli`. |
...
(`./ipify-cli <expr>`) and interactively (the REPL), and expressions compose the
