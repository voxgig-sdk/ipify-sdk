...
import (
	...
	omni "github.com/voxgig-sdk/ipify-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.IpifyError); is {
				out := map[string]any{
					"name":    "IpifyError",
...
func omniCtx(arg any, client *sdk.IpifySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.IpifySDK); is {
			client = live
