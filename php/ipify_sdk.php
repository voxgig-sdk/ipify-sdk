...
class IpifySDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = IpifyHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = IpifyHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!IpifyFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, IpifyFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return IpifyUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = IpifyHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = IpifyHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = IpifyHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new IpifyError($op . "_allow",
                "IpifySDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = IpifyHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new IpifyError("graphql_error",
                "IpifySDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function GetPublicIp($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_get_public_ip === null) {
                $this->_get_public_ip = new GetPublicIpEntity($this, null);
            }
            return $this->_get_public_ip;
        }
        return new GetPublicIpEntity($this, $data);
    }
