...
class IpifyNetsimFeature extends IpifyBaseFeature
{
    ...
    private function _simulate(IpifyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(IpifyContext $ctx, array $applied): void
    {
        $client = $this->client;
