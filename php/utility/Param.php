...
// Ipify SDK utility: param

require_once __DIR__ . '/../core/Helpers.php';

class IpifyParam
{
    public static function call(IpifyContext $ctx, mixed $paramdef): mixed
    {
        $point = $ctx->point;
        $spec = $ctx->spec;
        $match_val = $ctx->match;
        ...
        $akey = '';
        if ($point) {
            $alias_map = IpifyHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'alias'));
            if ($alias_map) {
