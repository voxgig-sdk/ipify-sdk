...
{
    private static function utility(): IpifyUtility
    {
        return new IpifyUtility();
    }
    ...
    private static function ctx(array $over = []): IpifyContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new IpifyContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new IpifyOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof IpifyError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new IpifyOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = IpifyMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new IpifyOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = IpifyMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new IpifyOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = IpifyMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IpifySpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = IpifyMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new IpifyResponse([]),
            'result' => new IpifyResult([]),
        ]);
        [, $err] = IpifyMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new IpifySpec(['step' => 's']),
            'response' => new IpifyResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new IpifyResult([]),
        ]);
        [, $err] = IpifyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new IpifySpec(['step' => 's']),
            'response' => new IpifyResponse(self::resp(200, ['v' => 1])),
            'result' => new IpifyResult([]),
        ]);
        [, $err] = IpifyMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new IpifySpec(['step' => 's']),
            'response' => new IpifyResponse(self::resp(200, ['v' => 2])),
            'result' => new IpifyResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        IpifyMakeResponse::call($ctx);
    ...
    {
        $preset = new IpifyResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new IpifySpec([]),
            'response' => new IpifyResponse([]),
            'result' => new IpifyResult([]),
        ...
        [$got, $err] = IpifyMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new IpifyResult([])]);
        [, $err] = IpifyMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new IpifySpec([]), 'result' => null]);
        [, $err] = IpifyMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IpifySpec(['step' => 's']),
            'result' => new IpifyResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new IpifyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IpifyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new IpifySpec(['step' => 's']),
            'result' => new IpifyResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new IpifyOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = IpifyMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new IpifyResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new IpifySpec([]),
            'result' => new IpifyResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = IpifyMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $utility = self::utility();
        $boom = new IpifyError('boom', 'boom');
        $utility->fetcher = function (IpifyContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IpifySpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = IpifyMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (IpifyContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = IpifyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(IpifyResponse::class, $response);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (IpifyContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new IpifySpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = IpifyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new IpifyResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new IpifySpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = IpifyMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = IpifyMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = IpifyMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new IpifyResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, IpifyDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new IpifyResult(['ok' => false])]);
        $this->expectException(IpifyError::class);
        IpifyDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new IpifyResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', IpifyMakeError::call($ctx, null));
    }
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new IpifyBaseFeature();
        $b = new IpifyBaseFeature();
        IpifyFeatureAdd::call($ctx, $a);
        IpifyFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): IpifyBaseFeature
    {
        $f = new IpifyBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    private static function auth_ctx(array $options, ?array $headers): IpifyContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new IpifySpec(['headers' => $headers]),
        ]);
    }
    ...
    {
        $ctx = self::ctx([
            'response' => new IpifyResponse(['headers' => null]),
            'result' => new IpifyResult([]),
        ]);
        IpifyResultHeaders::call($ctx);
