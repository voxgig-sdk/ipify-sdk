...
class FtCtrl extends IpifyControl
{
...
class FtClient
{
    ...
    private IpifyUtility $utility;

    public function __construct(IpifyUtility $utility, array $options)
    {
    ...
    public function get_utility(): IpifyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (IpifyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (IpifyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = IpifyConfig::make_feature($name);
    ...
    private static function buildUrl(IpifySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof IpifyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new IpifyResponse($fetched);
        }
    ...
    private function fail(IpifyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (IpifyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
