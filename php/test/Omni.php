...
//    only - a typed IpifyContext there would make every ctx
...
final class IpifyOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new IpifyContext($clean, null);
    ...
    public static function err_from_map($m): ?IpifyError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new IpifyError(is_string($code) ? $code : '', $msg);
    }
