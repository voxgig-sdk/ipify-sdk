...
class IpifyRetryFeature extends IpifyBaseFeature
{
    ...
    public function init(IpifyContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (IpifyContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(IpifyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(IpifyContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
