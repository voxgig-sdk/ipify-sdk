...
// Ipify SDK utility: make_error

require_once __DIR__ . '/../core/Operation.php';
require_once __DIR__ . '/../core/Result.php';
require_once __DIR__ . '/../core/Error.php';

class IpifyMakeError
{
    public static function call(?IpifyContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new IpifyContext([], null);
        }
        $op = $ctx->op ?? new IpifyOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new IpifyResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof IpifyError) ? $err->msg : (string)$err;
        $msg = "IpifySDK: {$opname}: {$errmsg}";
        ...
        $sdk_err = new IpifyError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof IpifyError) {
